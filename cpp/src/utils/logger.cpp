...
static const char* kLogLevelFullNames[] = { "Debug", "Info", "Warning", "Notice", "Error", "Critical", "Off", nullptr };
static const char* kLogLevelColourCodes[] = { kCodeColourBlack, kCodeColourGreen, kCodeColourOrange,
											  kCodeColourCyan, kCodeColourRed, kCodeColourRed, kCodeColourBlack, nullptr };

Logger::Logger() : m_logOutputType(eLogStdErr),
					m_pFileHandle(nullptr), m_pSecondaryStdErr(nullptr),
...
Logger::LogLevel Logger::convertStringToLogLevelEnum(const std::string& logLevelStr)
{
	if (logLevelStr == "critical")
		return Logger::eLevelCritical;
	else if (logLevelStr == "error")
		return Logger::eLevelError;
	else if (logLevelStr == "notice")
		return Logger::eLevelNotice;
	else if (logLevelStr == "warning")
		return Logger::eLevelWarning;
	else if (logLevelStr == "info")
		return Logger::eLevelInfo;
	else if (logLevelStr == "debug")
		return Logger::eLevelDebug;
	else
		return Logger::eLevelOff;
}
...
void Logger::outputLogItem(LogLevel itemLevel, const char* format, va_list args)
{
	...
	char szLogMessage[8192];

	// limit the length
	vsnprintf(szLogMessage, 7000, format, args);
