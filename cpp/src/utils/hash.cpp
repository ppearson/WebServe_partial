...
Hash::Hash() : m_hash(0ULL)
{

}

void Hash::addInt(int value)
{
	addData((const unsigned char*)&value, sizeof(int));
}

void Hash::addUInt(unsigned int value)
{
	addData((const unsigned char*)&value, sizeof(unsigned int));
}

void Hash::addULongLong(unsigned long long value)
{
	addData((const unsigned char*)&value, sizeof(unsigned long long));
}

void Hash::addUChar(unsigned char value)
{
	addData((const unsigned char*)&value, sizeof(unsigned char));
}

void Hash::addString(const std::string& value)
{
	unsigned int length = value.length();
	for (unsigned int i = 0; i < length; i++)
	{
		unsigned char c1 = value[i];
		addData(&c1, 1);
	}
}

void Hash::addData(const unsigned char* buffer, unsigned int length)
{
	while (length--)
	{
		m_hash = kCRCTab64[(m_hash ^ *(buffer++)) & 0xff] ^ (m_hash >> 8);
	}
}
